...
class CosmosDBServer(Server):
    ...
    def get_cosmos_client(self, account_name: str) -> CosmosClient:
        ...
        """
        # Use the env variable COSMOS_DB_NAME to construct the Cosmos DB URI
        cosmos_account_name = account_name or os.getenv("COSMOS_ACCOUNT_NAME")
        if not cosmos_account_name:
            raise ValueError("COSMOS_ACCOUNT_NAME environment variable is not set")

        cosmos_db_uri = f"https://{cosmos_account_name}.documents.azure.com:443/"
    ...
    async def _create_container(self, account_name: str, database_name: str, container_name: str, partition_key: str) -> Dict[str, Any]:
        ...
        """
        client = self.get_cosmos_client(account_name)
        return client.get_database_client(database_name).create_container_if_not_exists(
            id=container_name,
